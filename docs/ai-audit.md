...
|--------|---------|---------|--------|------|
| 状态分析 analyzeBattleState | ~220 | 105 | ~65% | 缺英雄法术/死兵/堆叠回避 |
| 撤退/投降 planUnitTurn Step 2 | ~160 | 30 | ~20% | 投降属范围外，撤退缺 _considerRetreat |
| 法术选择 selectBestSpell | ~958 | 137 | ~15% | 范围外(6/30+法术)，阈值/伤害启发式已对齐 |
| 射手决策 archerDecision | ~350 | ~80 | ~55% | 缺 AREA_SHOT / UnitRemover 评估 |
| 近战进攻 meleeUnitOffense | ~140 | ~90 | ~60% | 缺护城河/城墙接近/复合优先级 |
| 近战防守 meleeUnitDefense | ~400 | ~50 | ~30% | 缺堆叠回避/宽体侧面掩护/防区攻击 |
| 狂暴 berserkTurn | ~95 | ~35 | ~75% | 结构一致，细节简化 |
| 辅助函数 threat/pos_value | ~165 | 56 | ~60% | 缺双击反击折算/已移动惩罚 |
| **总计** | **~3050** | **801** | **~45%** | 含范围外法术；核心决策路径 ~60% |
...
| 3 | 敌军 AREA_SHOT 检测，威胁>10%时设 `_avoidStackingUnits` | :979-982 | 无 | ❌ |
...
| 6 | 死亡单位设 `_considerRetreat = true` 并跳过 strength 累加 | :1009-1012 | 无 `_considerRetreat` | ❌ |
| 7 | `initialUnitCount < 4` → `_considerRetreat = true` | :1004 | 无 | ❌ |
...
| # | 原版行为 | 源码位置 | 我们的实现 | 状态 |
|---|---------|---------|-----------|------|
| 15 | `_commander` 有射手时，`commanderMaximumSpellDamageValue` 加入 `_myShootersStrength` | :1109-1111 | 无 | ❌ |
| 16 | 敌方英雄法术威胁 `_enemySpellStrength = enemyCommander->GetMagicStrategicValue` | :1114-1115 | 无 | ❌ |
| 17 | 敌方英雄法术伤害加入 `_enemyShootersStrength` | :1116 | 无 | ❌ |
...
| 25 | `_avoidStackingUnits` 传递到 meleeUnitDefense | :1056 | 无 | ❌ |
...
| 9 | AOE 伤害法术（Chain Lightning/Meteor 等）遍历溅射目标 | :211-248 | 无 AOE 法术 | ❌ 范围外 |
...
| # | 法术 | 原版行号 | 状态 |
|---|------|---------|------|
| 22 | Mass 变体（MassSlow/Haste/Bless/Curse） | :562-620 | ❌ 范围外 |
| 23 | Blind / Paralyze | :386-470 | ❌ 范围外 |
| 24 | Berserk | :474-486 | ❌ 范围外 |
| 25 | Hypnotize | :488-490 | ❌ 范围外 |
| 26 | Disrupting Ray | :296-316 | ❌ 范围外 |
| 27 | Bloodlust / Stone Skin / Steel Skin | :477,492-494 | ❌ 范围外 |
| 28 | Antimagic / Mirror Image / Shield | :500-556 | ❌ 范围外 |
| 29 | Dragon Slayer | :726-748 | ❌ 范围外 |
| 30 | Teleport | :752-848 | ❌ 范围外 |
| 31 | Earthquake | :852-880 | ❌ 范围外 |
| 32 | Dispel | :156-158 | ❌ 范围外 |
| 33 | Summon | :882-906 | ❌ 范围外 |
| 34 | Resurrect | :624-640 | ❌ 范围外 |
...
| 9 | AREA_SHOT 单位：计算溅射优先级（受影响单位 evaluateThreatForUnit 总和），含友军伤害检查 | :327-375 | 无 | ❌ |
...
| # | 原版行为 | 源码位置 | 我们的实现 | 状态 |
|---|---------|---------|-----------|------|
| 1 | `evaluatePotentialAttackPositions` 预计算所有攻击位置价值 | :202-270 | 无预计算，攻击时实时算 | ⚠️ 简化 |
| 2 | `BestAttackOutcome` 复合优先级：canAttackImmediately > positionValue > attackValue | :297-350 | 简化为 `strength + pos_value` | ⚠️ 近似 |
| 3 | `optimalAttackValue`：含双格攻击溅射价值 + 全邻接攻击价值 | :160-199 | 无溅射/全邻接评估 | ⚠️ 简化 |
...
| 7 | 护城河：路径终点在护城河 → 停在护城河（更多下回合自由度） | :1598-1603 | 无 | ❌ |
...
| # | 原版行为 | 源码位置 | 我们的实现 | 状态 |
|---|---------|---------|-----------|------|
| 10 | 无可达目标且 `_attackingCastle` → 向城墙 `cellsUnderWallsIndexes` 移动 | :1618-1638 | 无 | ❌ |
...
| 3 | `_avoidStackingUnits`：掩护位置远离射手/其他掩护单位 | :1773-1825 | 无 | ❌ |
| 4 | 宽体侧面掩护优先方向逻辑 | :1782-1810 | 无 | ❌ |
| 5 | 射手被堵 → 攻击堵截的敌人（`BestAttackOutcome`） | :1855-1875 | 同（攻击最近敌人） | ⚠️ 近似 |
| 6 | 无视反击单位 + AREA_SHOT 友军：掩护时主动攻击邻接敌人 | :1905-1920 | 无 | ❌ |
| 7 | 2回合内不可达但有即击目标 → 忽略远距离射手 | :1843-1847 | 无 | ⚠️ 近似 |

### 6.2 防区攻击（第二阶段）

| # | 原版行为 | 源码位置 | 我们的实现 | 状态 |
|---|---------|---------|-----------|------|
| 8 | 无射手可掩护 → 在己方半场找目标攻击（`isPositionLocatedInDefendedArea` 过滤） | :1930-1960 | 无此阶段（直接 fallback 到 _offense） | ⚠️ 简化 |
...
| 2 | `isDoubleAttack()`：反击折算第二击伤害 | :1030-1057 | 无 | ❌ |
...
| 11 | `TR_MOVED` 已行动 → threat /= 1.25 | :1138 | 无 | ❌ |

### 8.2 evaluatePotentialAttackPositions（我们的 `pos_value()`）

| # | 原版行为 | 源码位置 | 我们的实现 | 状态 |
|---|---------|---------|-----------|------|
| 1 | 遍历每个敌人的距离格，构建位置→攻击价值映射 | :202-270 | 实时计算，无预建映射 | ⚠️ 简化 |
| 2 | 宽体攻击者可达距离 2 的位置 | :226 | 仅用 head 邻接 | ⚠️ 简化 |
...
> M6c 常量对齐后，核心决策路径（排除「范围外」的 22 项）：
> ✅ 47 / (111-22) = 47/89 ≈ **53%** 已对齐
> ⚠️ 32 / 89 ≈ **36%** 近似
> ❌ 10 / 89 ≈ **11%** 暂缺（含 A9 防守第二阶段等）
>
> 综合保真度估算：47 完全对齐 + 32 近似（权重×0.6）≈ 47+19 = 66 / 89 ≈ **74% 决策行为覆盖**
> 若含范围外项目（法术/能力/镜像等）：111 总项，对齐+近似 ≈ **71% 行为覆盖**
>
> 规则复刻保真度（以游戏规则子系统为分母）约 **~95%**（MILESTONES.md）。
...
| A9 | 缺防守第二阶段（防区内攻击） | 在 planner._defense fallback 前 | 中 | 📋 留后续 |
