...
> AI 决策层 A 系列 A1–A4 全部完成，~96% 决策行为覆盖（126 条审计，102✅ / 11⚠️ / 13❌范围外），352 测试。
>
...
- [x] §7-#1：狂暴目标排序改用 head-to-head 距离（匹配 C++ `GetNearestTroops`）
- [x] §7-#5：`_can_attack_from_pos` 验证宽体朝向 + 护城河攻击限制
- [x] §9-#4：`isDisableCastSpell` 改标 ✅（功能等价，`can_cast`+spell value 已覆盖）
- [x] §9-#1/§1-#18：修正 2 项陈旧审计标注
- [x] 最终审计更新 `ai-audit.md`，覆盖率 96% → **97%**

> 2 项修复 + 1 项改标 + 2 项陈旧修正，15 新测试，352 全绿，覆盖率 96% → ~97%。
